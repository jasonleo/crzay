...
uint8_t MPU6050_getDeviceID(void) {
    ...
    i2c_read_bytes(devAddr, MPU6050_RA_WHO_AM_I, 1, buffer);
    return buffer[0];
}
...
uint8_t MPU6050_readMemoryByte(void) {
    i2c_read_bytes(devAddr, MPU6050_RA_MEM_R_W, 1 , buffer);
    return buffer[0];
}

// XG_OFFS_USR* registers
int16_t MPU6050_getXGyroOffset(void) {
    i2c_read_bytes(devAddr, MPU6050_RA_XG_OFFS_USRH, 2, buffer);
    return (((int16_t)buffer[0]) << 8) | buffer[1];
}
int16_t MPU6050_getYGyroOffset(void) {
    i2c_read_bytes(devAddr, MPU6050_RA_YG_OFFS_USRH, 2, buffer);
    return (((int16_t)buffer[0]) << 8) | buffer[1];
}
int16_t MPU6050_getZGyroOffset(void) {
    i2c_read_bytes(devAddr, MPU6050_RA_ZG_OFFS_USRH, 2, buffer);
    return (((int16_t)buffer[0]) << 8) | buffer[1];
}
...
uint8_t MPU6050_writeMemoryBlock(const uint8_t *data, uint16_t dataSize, uint8_t bank, uint8_t address, uint8_t verify, uint8_t useProgMem) {
    ...
    for (i = 0; i < dataSize;) {
        ...
        i2c_write_bytes(devAddr, MPU6050_RA_MEM_R_W, chunkSize, tprogBuffer);
...
void MPU6050_setXGyroOffset(int16_t offset) {
    ...
    i2c_write_bytes(devAddr, MPU6050_RA_XG_OFFS_USRH, 2, buffer);
}
void MPU6050_setYGyroOffset(int16_t offset) {
	buffer[0] = offset>>8;
	buffer[1] = offset&0x00ff;
    i2c_write_bytes(devAddr, MPU6050_RA_YG_OFFS_USRH, 2, buffer);
}

void MPU6050_setZGyroOffset(int16_t offset) {
    buffer[0] = offset>>8;
	buffer[1] = offset&0x00ff;
    i2c_write_bytes(devAddr, MPU6050_RA_ZG_OFFS_USRH, 2, buffer);
}
...
uint16_t MPU6050_getFIFOCount(void) {
    i2c_read_bytes(devAddr, MPU6050_RA_FIFO_COUNTH, 2, buffer);
...
void MPU6050_readMemoryBlock(uint8_t *data, uint16_t dataSize, uint8_t bank, uint8_t address) {
    ...
    for (i = 0; i < dataSize;) {
        ...
        i2c_write_bytes(devAddr, MPU6050_RA_MEM_R_W, chunkSize, data + i);
...
uint8_t MPU6050_getIntStatus(void) {
    return i2c_read_onebyte(devAddr, MPU6050_RA_INT_STATUS);
}
...
uint8_t MPU6050_getOTPBankValid(void) {
	uint8_t  temp = i2c_read_onebyte(devAddr, MPU6050_RA_XG_OFFS_TC);
...
int8_t MPU6050_getYGyroOffsetTC(void) {
	uint8_t  temp = i2c_read_onebyte(devAddr, MPU6050_RA_YG_OFFS_TC);
	temp &= 0x3F;
    return temp;
}
...
int8_t MPU6050_getZGyroOffsetTC(void) {
	uint8_t  temp = i2c_read_onebyte(devAddr, MPU6050_RA_ZG_OFFS_TC);
	temp &= 0x3F;
    return temp;
}
